...
class CouponController:
    ...
    def get_coupons_by_user(self, user_id):
        user_coupons = self.db.query(CouponUser).filter_by(client_id=user_id).all()
    ...
    def delete_coupons_by_client(self, coupon_id, user_id):
        coupon = self.db.query(CouponUser).filter_by(id=coupon_id, client_id=user_id).first()
        if not coupon:
            return False
    ...
    def pick_up_coupon_by_client_id(self, data):
        ...
        coupon = self.db.query(Coupon).filter_by(title=coupon_title).first()
        if not coupon:
            return {'error': 'Cupom não encontrado.'}, 404

        existing = self.db.query(CouponUser).filter_by(client_id=client_id, coupon_id=coupon.id).first()
        if existing:
